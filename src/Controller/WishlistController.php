...
final class WishlistController extends AbstractController
{
    ...
    // public function wishlistToggle(Request $request, Game $game, EntityManagerInterface $entityManager , WishlistItemRepository $wishlistItemRepository): Response {
    //     $user = $this->getUser();
    //     if (!$user) {
    ...
    public function toggleWishlist(Game $game, WishlistItemRepository $wishlistRepo, EntityManagerInterface $em, Security $security): Response
    {
        $user = $security->getUser();
        $wishlistItem = $wishlistRepo->findOneBy(['user' => $user, 'game' => $game]);
        ...
        // Retourner le fragment HTML pour Turbo
        return $this->render('wishlist/_wishlist_button.html.twig', [
            'game' => $game,
            'wishlist' => $wishlist,
        ]);
    ...
    #[Route('/wishlist', name: 'app_wishlist')]
    public function wishlist(Security $security): Response
    {
        $user = $security->getUser();
        $wishlistItems = $user ? $user->getWishlistItems() : [];
